...
export function nearestObservedStation(latitude:number,longitude:number){return stations.reduce((closest,station)=>distanceKm(latitude,longitude,station[2],station[3])<distanceKm(latitude,longitude,closest[2],closest[3])?station:closest)}

export async function getObservedProfile(latitude:number,longitude:number):Promise<SoundingProfile>{
  const station=nearestObservedStation(latitude,longitude),end=new Date(),start=new Date(end.getTime()-42*3_600_000);
  const params=new URLSearchParams({sts:start.toISOString(),ets:end.toISOString(),station:station[0],format:"comma"});
  const response=await fetch(`https://mesonet.agron.iastate.edu/cgi-bin/request/raob.py?${params}`);
